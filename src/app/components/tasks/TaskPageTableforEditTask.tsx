...
const ThemePageTable = forwardRef((props, ref) => {
  ...
  const handleEditTitle = async (updatedText) => {
    ...
    formData.append(
      "labels",
      selectedLabels.length > 0 ? (selectedLabels as any) : "0"
    );
  ...
  const handleAgentSelect = (agentId) => {
    ...
    const isSelected =
      ...
      selectedAgents.some((agent) => agent.user_id === agentId);

    if (isSelected) {
      // Remove the agent if they are already selected
      setSelectedAgents(
        selectedAgents.filter((agent) => agent.user_id !== agentId)
      );
      ...
      setSelectedAgents([
        ...selectedAgents,
        { ...selectedAgent, user_id: selectedAgent.agent_id },
      ]);
    }
  };
                                      ...
                                      <div className="flex items-center gap-10">
                                        ...
                                        {/* <div
                                          onClick={(e) => {
                                            ...
                                            navigate(
                                              `/${row?.id}/${project_id}/subTask/detail/${row.id}`
                                            );
                                          }}
                                        > */}
                                        <TruncateText
