...
import { accManagerHideSchema, accManagerSchema } from "src/formSchema";
...
function AddAccountManagerModel({
  ...
  const onSubmit = async (values: AccManagerType, { resetForm }) => {
    ...
    if (accountManager_id) {
      ...
      formData.append("zipcode", values?.zipcode?.toString()?.trim());
      ...
      formData.append("city", values?.city?.trim() || "");
      formData.append("state", values?.state?.trim() || "");
      formData.append("zipcode", values?.zipcode?.toString()?.trim() || "0");
      formData.append("country", values?.country?.trim() || "");
      formData.append("role_permission_id", values?.role_permission_id);
        ...
        <>
          ...
          <div className="flex gap-20 w-[50%]">
            ...
            >
              {list?.length > 0 ? (
                list?.map((item) => (
                  <StyledMenuItem key={item.name} value={item.id}>
                    {item.name}
                  </StyledMenuItem>
                ))
              ) : (
