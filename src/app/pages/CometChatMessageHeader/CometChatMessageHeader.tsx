...
import { useCometChatErrorHandler, useRefSync } from "../../../utils/CometChatCustomHooks";
...
import ConversationSummaryIcon from 'assets/icons/conversation_summary.svg';
import { CometChatUIEvents } from "../events/CometChatUIEvents";
import { CometChatConversationSummary } from "../CometChatConversationSummary/CometChatConversationSummary";
    ...
    const updateSubtitle = useCallback(() => {
        try {
            ...
            if (user && !hideUserStatus) {
                ...
                const membersText = (count > 1 ? "Members" : "Member");
    ...
    const updateLastActiveInfo = (user: CometChat.User) => {
        try {
            if (user.getStatus() === CometChatUIKitConstants.userStatusType.online) {
                setSubtitleText(user.getStatus().toUpperCase());
            } else {
                if (user.getLastActiveAt()) {
                    ...
                    if (timeDifferenceInMinutes < 60) {
                        if (Math.ceil(timeDifferenceInMinutes) == 0 || Math.ceil(timeDifferenceInMinutes) == 1) {
                            setSubtitleText(`"Last see 1 minute ago`);
                        } else {
        ...
        try {
            ...
            if (isTypingForCurrentChat(typing)) {
                ...
                if (loggedInUser?.getUid() == receiverId) {
                    ...
                    setSubtitleText("Typing");
                }
                if (groupRef?.current?.getGuid() === receiverId) {
                    isTypingRef.current = true;
                    setSubtitleText(`${sender?.getName()}: Typing`);
                }
            }
