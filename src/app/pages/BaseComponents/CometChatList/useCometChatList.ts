...
export function useCometChatList(args: Args) {
  ...
  useEffect(
    ...
    () => {
      if (
        ...
        !intersectionObserverBottomTargetRef.current
      ) {
        return;
      }
      ...
      async function observerCallBack(entry: IntersectionObserverEntry[]) {
        ...
        if (
          didComponentScrollToBottomRef.current &&
  ...
  useEffect(
    ...
    () => {
      if (
        !intersectionObserverRootRef.current ||
        !intersectionObserverTopTargetRef.current
      ) {
        return;
      }
    ...
    () => {
      ...
      if (
        ...
        if (
          Math.round(
            intersectionObserverRootRef.current.scrollHeight -
              scrollHeightTupleRef.current[1]
          ) !== 0
        ) {
