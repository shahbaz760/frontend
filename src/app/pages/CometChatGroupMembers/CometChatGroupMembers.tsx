...
import {
  ...
  useRefSync,
  useStateRef,
} from "src/utils/CometChatCustomHooks";
...
import { GroupMemberUtils } from "src/utils/GroupMemberUtils";
import { Placement, SelectionMode, States } from "src/app/pages/Enums/Enums";
import { CometChatActionsIcon, CometChatOption } from "src/utils/modals";
...
import emptyIcon from "/assets/icons/groups_empty_state.svg";
import emptyIconDark from "/assets/icons/groups_empty_state_dark.svg";
import errorIcon from "/assets/icons/list_error_state_icon.svg"
import errorIconDark from "/assets/icons/list_error_state_icon_dark.svg"
import { getThemeMode } from "src/utils/util";
...
export function CometChatGroupMembers(props: GroupMembersProps) {
  ...
  const titleRef = useRef<string>("Group Members");
  const searchPlaceholderTextRef = useRef<string>("Search");
  /**
      ...
      if (typeof groupMemberOptions === "string" && groupMemberOptions !== "participant") {
        return (
          <div className={`cometchat-group-members__trailing-view-options cometchat-group-members__trailing-view-options-${groupMemberOptions}`}>
            {(groupMemberOptions.toUpperCase())}
          </div>
        );
      }

      return <div className={`cometchat-group-members__trailing-view-options cometchat-group-members__trailing-view-options-${groupMember?.getScope()}`}>{groupMember?.getScope() !== CometChatUIKitConstants.groupMemberScope.participant && (groupMember?.getScope().toUpperCase())}</div>
    } catch (error) {
      ...
      return (
        <div className="cometchat-group-members__empty-state-view">
          ...
          <div className="cometchat-group-members__empty-state-view-body">
            <div className="cometchat-group-members__empty-state-view-body-title">{"No Group Member Available"}</div>
            <div className="cometchat-group-members__empty-state-view-body-description">{"Add contacts to see them listed here."}</div>
          </div>
        </div>
      )
    } catch (error) {
      ...
      return (
        <div className="cometchat-group-members__error-state-view">
          ...
          <div className="cometchat-group-members__error-state-view-body">
            <div className="cometchat-group-members__error-state-view-body-title">{"OOPS!"}</div>
            <div className="cometchat-group-members__error-state-view-body-description">{"Looks like something went wrong"}
            </div>
          </div>
        </div>
      )
