...
import {
  ...
  MouseEventSource,
} from "src/app/pages/Enums/Enums";
import { CometChatUIEvents } from "src/app/pages/events/CometChatUIEvents";
import { CometChatUtilityConstants } from "src/app/pages/constants/CometChatUtilityConstants";
  ...
  protected addMentionsSpan(inputText: string) {
    if (this.cometChatUserGroupMembers) {
      ...
      for (let i = 0; i < this.cometChatUserGroupMembers?.length; i++) {
        ...
        if (
          this.cometChatUserGroupMembers[i].getUid() ===
        ...
        if (this.messageBubbleAlignment === MessageBubbleAlignment.left) {
          ...
          this.messageBubbleAlignment === MessageBubbleAlignment.right
        ) {
      ...
      if (classList[i] in this.mentionsCssClassMapping!) {
        element.addEventListener("click", (event: Event) => {
          ...
          CometChatUIEvents.ccMouseEvent.next({
            body: {
              CometChatUserGroupMembersObject: this.mentionsCssClassMapping[
                classList[i]
              ],
    ...
    setTimeout(
      (event) => {
        ...
        if (visibility === MentionsVisibility.both) {
          ...
          visibility === MentionsVisibility.usersConversationOnly &&
          composerConfig.user
        ) {
          this.keyDownCallBack(precedingText);
        } else if (
