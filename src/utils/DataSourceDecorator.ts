...
import { CometChatUrlsFormatter } from "./formatters";
import { CometChatMentionsFormatter } from "./formatters";
...
export abstract class DataSourceDecorator implements DataSource {
  ...
  getTextMessageContentView(
    ...
    ).getTextMessageContentView(message, alignment, additionalConfigurations);
  }
  ...
  getAllTextFormatters(
    formatterParams: additionalParams
  ): CometChatTextFormatter[] {
    let formatters = [];
    const mentionsFormatter = formatterParams.disableMentions
      ? null
      : (this.dataSource ?? new MessagesDataSource()).getMentionsTextFormatter(
          formatterParams
        );
    const urlTextFormatter = (
      this.dataSource ?? new MessagesDataSource()
